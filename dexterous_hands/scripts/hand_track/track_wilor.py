...
class LatestPose:
    ...
    """
    kpts_3d: Optional[np.ndarray] = None    # (21, 3) MANO local frame
...
def inference_thread(pipe, latest_frame: LatestFrame, latest_pose: LatestPose,
    ...
    while not stop_event.is_set():
        ...
        with latest_pose.lock:
            latest_pose.kpts_3d = joints_3d
            latest_pose.kpts_2d = kpts_2d
            latest_pose.is_right = is_right
            latest_pose.capture_ts = capture_ts # type: ignore
...
def draw_overlay(frame_bgr: np.ndarray, kpts_2d: np.ndarray,
                 hud_lines: list | None = None) -> np.ndarray:
...
def main():
    ...
    parser.add_argument("--record", type=str, default=None,
                        help="If given, record session to this pickle file")
    parser.add_argument("--max-frames", type=int, default=600,
                        help="Stop recording after this many frames (default 600 = ~30s at 20Hz)")
    args = parser.parse_args()

    # Initialise recording state
    recording = []
    recording_active = args.record is not None
    if recording_active:
        print(f"[record] will save to {args.record}, max {args.max_frames} frames")
    ...
    try:
        while not stop_event.is_set():
            with Timer(stages["loop_total"]):
                ...
                if recording_active:
                    with filtered_pose.lock:
                        joints_3d_snap = (filtered_pose.kpts_3d.copy()
                                          if filtered_pose.kpts_3d is not None else None)
                        kpts_2d_snap = (filtered_pose.kpts_2d.copy()
                                        if filtered_pose.kpts_2d is not None else None)
                        is_right_snap = filtered_pose.is_right
                        capture_ts_snap = filtered_pose.capture_ts
                    if joints_3d_snap is not None:
                        recording.append({
                            "joints_3d": joints_3d_snap,
                            "kpts_2d": kpts_2d_snap,
                            "is_right": is_right_snap,
                            "capture_ts": capture_ts_snap,
                            "frame_idx": len(recording),
                        })
                        if len(recording) >= args.max_frames:
                            print(f"[record] hit max frames, saving and stopping")
                            stop_event.set()
        ...
        if recording_active and len(recording) > 0:
            ...
            print(f"[record] saved {len(recording)} frames to {args.record}")
